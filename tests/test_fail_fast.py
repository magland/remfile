...
class _Handler(BaseHTTPRequestHandler):
    ...
    error_status = None
    # Status to return for the file-size probe (the GET with no Range header).
    probe_error_status = None
    request_count = 0
    ...
    def do_GET(self):  # noqa: N802
        ...
        if not rng:
            ...
            if _Handler.probe_error_status is not None:
                self.send_response(_Handler.probe_error_status)
...
def server():
    ...
    _Handler.probe_error_status = None
...
def test_permanent_errors_when_opening_fail_immediately(server, status):
    """Opening a missing file, or one whose URL has expired, must not back off.

    The file-length request retries transient failures, so without a permanent
    error check a 404 would spend ~25 seconds backing off before reporting that
    the file simply isn't there.
    """
    _Handler.probe_error_status = status
    _Handler.request_count = 0
...
def test_transient_errors_when_opening_are_still_retried(server):
    """A 500 on the file-length request is transient and must still be retried."""
    _Handler.probe_error_status = 500
    _Handler.request_count = 0
